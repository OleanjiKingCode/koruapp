...
export default function AppealsPage() {
  ...
  const allAppeals = data?.summons || [];

  // Share modal state
  const [shareModalOpen, setShareModalOpen] = useState(false);
  const [selectedAppeal, setSelectedAppeal] = useState<Summon | null>(null);
  ...
  const handleShareAppeal = (appeal: Summon) => {
  ...
  const filteredAppeals = useMemo(() => {
    ...
    return sorted;
  }, [allAppeals, sortBy, sortDirection]);
  ...
  const handleCreateAppeal = async () => {
    if (!session?.user?.id) {
      setError("You must be logged in to create an appeal");
      return;
    }
    ...
    if (!requestText.trim()) {
      setError("Please write a message for your appeal");
      return;
    }
                          ...
                          <div>
                            <h2 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">
                              {step === "info"
                                ? "Create an Appeal"
                                : selectedProfile
                                ? `Appeal to ${selectedProfile.name.split(" ")[0]}`
                                : "Create Appeal"}
                            </h2>
        ...
        <AnimatePresence mode="wait">
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="Be the first to create an appeal and rally the community!"
              ctaText="Create Appeal"
              variant="card"
            />
          ) : viewMode === "treemap" ? (
            <TreemapView
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Creator info */}
              {appeal.creatorUsername && (
                <div className="flex items-center gap-1.5 mt-1">
                  <span className="text-xs text-neutral-400">Created by</span>
                  {appeal.creatorProfileImage ? (
                    <img
                      src={appeal.creatorProfileImage}
                      alt={appeal.creatorName || appeal.creatorUsername}
                      className="w-4 h-4 rounded-full"
                    />
                  ) : (
                    <div className="w-4 h-4 rounded-full overflow-hidden">
                      <AvatarGenerator
                        seed={appeal.creatorUsername}
                        size={16}
                      />
                    </div>
                  )}
