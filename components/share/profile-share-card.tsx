          ...
          <div className="relative z-10 h-full p-8 flex flex-col">
            ...
            <div className="flex-1 flex items-end">
              <div className="flex gap-8">
                <div>
                  <p className="text-3xl font-bold text-white">{userData.points.toLocaleString()}</p>
                  <p className="text-xs text-white/40 uppercase tracking-wider">Points</p>
                </div>
                <div>
                  <p className="text-3xl font-bold text-[#c385ee]">{userData.level}</p>
                  <p className="text-xs text-white/40 uppercase tracking-wider">Level</p>
                </div>
                <div>
                  <p className="text-3xl font-bold text-[#9deb61]">{userData.badges.length}</p>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-white/5">
              ...
              <p className="text-white/30 text-xs">Member since {userData.joinDate}</p>
            </div>
          </div>
        </div>
      );
    ...
    if (variant === "gradient") {
      return (
        <div
          ...
          style={{ 
            background: "linear-gradient(135deg, #c385ee 0%, #dab079 50%, #9deb61 100%)",
            fontFamily: "system-ui, sans-serif"
          }}
        >
          {/* Noise texture */}
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="flex gap-10 bg-black/20 backdrop-blur-sm rounded-2xl px-8 py-4">
                <div className="text-center">
                  <p className="text-4xl font-black text-white">{userData.points.toLocaleString()}</p>
                  <p className="text-xs text-white/70 font-medium uppercase tracking-wider">Points</p>
                </div>
                <div className="w-px bg-white/20" />
                <div className="text-center">
                  <p className="text-4xl font-black text-white">{userData.level}</p>
                  <p className="text-xs text-white/70 font-medium uppercase tracking-wider">Level</p>
                </div>
              </div>
            </div>
      ...
      return (
        ...
        >
          ...
          <div className="absolute inset-0 rounded-3xl" style={{
            background: "linear-gradient(45deg, #c385ee, #dab079, #9deb61, #c385ee)",
          ...
          <div className="relative z-10 h-full p-8 flex flex-col">
            ...
            <div className="flex items-start gap-5">
              <div className="relative">
                <div className="absolute -inset-1 bg-gradient-to-r from-[#c385ee] via-[#dab079] to-[#9deb61] rounded-2xl blur opacity-75" />
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {userData.badges.map((badge) => (
                <span 
                  ...
                  style={{
                    background: "linear-gradient(135deg, rgba(195,133,238,0.2) 0%, rgba(157,235,97,0.1) 100%)",
            ...
            <div className="flex-1 flex items-end">
              <div className="w-full grid grid-cols-3 gap-4">
                <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                  <p className="text-2xl font-bold text-[#c385ee]">{userData.stats.totalSpent}</p>
                  <p className="text-xs text-white/40">Total Spent</p>
                </div>
                <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                  <p className="text-2xl font-bold text-[#dab079]">{userData.stats.activeChats}</p>
                  <p className="text-xs text-white/40">Active Chats</p>
                </div>
                <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                  <p className="text-2xl font-bold text-[#9deb61]">{userData.stats.appealsCreated}</p>
        ...
        <div className="relative z-10 h-full p-8 flex flex-col">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {userData.badges.map((badge, index) => (
              <span 
                ...
                style={{
                  background: index === 0 ? "rgba(195,133,238,0.2)" : 
                              index === 1 ? "rgba(218,176,121,0.2)" : "rgba(157,235,97,0.2)",
                  color: index === 0 ? "#c385ee" : index === 1 ? "#dab079" : "#9deb61"
                }}
              >
          ...
          <div className="flex-1 flex items-center">
            <div className="w-full grid grid-cols-2 gap-4">
              <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-5 border border-white/5">
                <p className="text-3xl font-bold text-white mb-1">{userData.points.toLocaleString()}</p>
                <p className="text-xs text-white/50 font-medium">Koru Points</p>
              </div>
              <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-5 border border-white/5">
                <p className="text-3xl font-bold text-[#9deb61] mb-1">{userData.stats.totalSpent}</p>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-white/10">
            <div className="flex items-center gap-3">
              ...
              <span className="text-white/70 text-sm font-semibold">koruapp.xyz</span>
            </div>
